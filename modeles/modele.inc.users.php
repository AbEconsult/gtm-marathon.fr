...
echo "<br> 3) je passe dans - modeles/modele.inc.users.php - pour la 1ère fois et lance connexion()";$modele_inc_users ++;
// $modele_inc_users =" 5) modele_inc_users - "; echo $modele_inc_users;
connexion();echo "<br> 5) j'ai lancé connexion() qui se trouve dans modele.inc et retourne sur controlConnexion ";$modele_inc_users ++;
...
function insUser(string $nom, string $user_can, string $email, string $mdp, int $id_service)
{
    ...
    $sql = "INSERT INTO user (username,username_canonical,email,password,roles) VALUES (?,?,?,?,?,?)";
 ...
 * @return array
 */
function searchUser(string $nom, string $user_can, string $mail)
{
    echo "<br> je passe dans function searchUsers - modeles/modele.inc.users.php - pour la 1ère fois ";
    $mysql = connexion();

    $sql = "SELECT * FROM user WHERE username LIKE :nom AND username_canonical LIKE :user_can AND us_mail LIKE :mail";


    $reponse = $mysql->prepare($sql);

    $reponse->execute([":nom" => '%' . $nom . '%', ":user_can" => '%' . $user_can . '%', ":mail" => '%' . $mail . '%']);
